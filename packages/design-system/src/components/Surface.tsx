...
export function Card({ children, className = "", ...props }: HTMLAttributes<HTMLElement> & { children: ReactNode }) {
  return <section className={`rounded-[18px] border border-white/80 bg-white shadow-[0_2px_8px_rgba(34,42,57,.055)] ${className}`} {...props}>{children}</section>;
}
...
export function MetricCard({ label, value, detail, icon = "↗", tone = "neutral" }: { label: string; value: ReactNode; detail?: ReactNode; icon?: ReactNode; tone?: "neutral" | "brand" | "success" | "info" }) {
  const tones = { neutral: "bg-[#f6f7f9] text-ink", brand: "bg-brand-soft text-brand", success: "bg-pine-soft text-pine", info: "bg-info-soft text-info" };
