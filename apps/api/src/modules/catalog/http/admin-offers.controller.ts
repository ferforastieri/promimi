...
export const adminOffersController: FastifyPluginAsync = async (app) => {
  app.get("/admin/offers", { preHandler: requireStaff }, async () => ({
    data: (await catalogRepository.adminOffers()).map(presentOffer),
  }));
  app.post(
    "/admin/offers",
    ...
    async (request, reply) => {
      const offer = await createOffer({
        ...offerInputSchema.parse(request.body),
  ...
  app.patch(
    ...
    async (request) => {
      ...
      return {
        data: await updateOffer(
          id,
          offerInputSchema.partial().parse(request.body),
        ),
      };
    },
  );
