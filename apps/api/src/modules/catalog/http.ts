...
import { categories, db, integrations, offers, outboxEvents, publications } from "@promimi/database";
import { z } from "zod";
...
export async function registerCatalogHttp(app: FastifyInstance) {
  ...
  app.post("/admin/categories", { preHandler: requireStaff }, async (request, reply) => {
    const input = z.object({ name: z.string().trim().min(2).max(80), description: z.string().trim().max(500).optional().nullable(), isActive: z.boolean().default(true) }).parse(request.body); const slug = slugify(input.name);
    if (await db.query.categories.findFirst({ where: eq(categories.slug, slug) })) return reply.code(409).send({ error: "CATEGORY_EXISTS", message: "Já existe uma categoria com este nome." });
    const [category] = await db.insert(categories).values({ ...input, slug }).returning(); return reply.code(201).send({ data: category });
  });
