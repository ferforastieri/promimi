...
export const communityController: FastifyPluginAsync = async (app) => {
  app.get("/offers/:id/comments", async (request) => {
    ...
    return {
      data: rows.map((comment) => ({
        ...
        author: comment.user.name ?? "Visitante Promimi",
      })),
    };
  });
  app.post(
  ...
  app.post(
    "/offers/:id/comments",
    { preHandler: requireUser, config: { rateLimit: rateLimits.comment } },
    async (request) => {
      const { id } = request.params as { id: string };
      const body = z
        .object({ body: z.string().trim().min(2).max(1500) })
        .parse(request.body);
      const [comment] = await communityRepository.comment(
        id,
        request.user.id,
        createCommentInput(body.body),
      );
      return { data: comment };
    },
  );
  app.post(
    "/comments/:id/report",
    { preHandler: requireUser, config: { rateLimit: rateLimits.report } },
    async (request) => {
      const { id } = request.params as { id: string };
      const body = z
  ...
  app.get("/admin/comments", { preHandler: requireStaff }, async () => ({
    data: await communityRepository.moderationQueue(),
  }));
  app.patch(
    "/admin/comments/:id",
    ...
    async (request) => {
      const { id } = request.params as { id: string };
      const { isHidden } = z
        .object({ isHidden: z.boolean() })
        .parse(request.body);
      const [comment] = await communityRepository.moderate(id, isHidden);
      return { data: comment };
    },
  );
};
