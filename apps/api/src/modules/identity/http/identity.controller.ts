...
import { deliverMail, passwordResetMail, verificationMail } from "../../../shared/notifications/mailer.js";
...
export const identityController: FastifyPluginAsync = async (app) => {
  ...
  app.post("/auth/reset-password", { config: { rateLimit: rateLimits.login } }, async (request, reply) => {
    ...
    if (!record) return reply.code(400).send({ error: "INVALID_TOKEN", message: "Este link expirou ou já foi utilizado." });
    await identityService.resetPassword(record.userId, record.id, body.password);
    return { ok: true };
  });
  ...
  app.post("/admin/totp/setup", { preHandler: requireStaff }, async (request, reply) => {
    ...
    const secret = authenticator.generateSecret(); await identityService.saveTotpSecret(request.user.id, encryptSecret(secret, encryptionKey()));
  ...
  app.post("/admin/totp/confirm", { preHandler: requireStaff }, async (request, reply) => {
    ...
    await identityService.enableTotp(request.user.id); return { ok: true };
  });
  app.get("/admin/users", { preHandler: requireStaff }, async () => ({ data: await identityService.activeUsers() }));
