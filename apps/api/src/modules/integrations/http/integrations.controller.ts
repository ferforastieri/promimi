...
export const integrationsController: FastifyPluginAsync = async (app) => {
  ...
  app.post("/admin/integrations/whatsapp/validate", { preHandler: requireStaff }, async (_request, reply) => {
    const integration = await integrationRepository.find("whatsapp"); if (!integration?.credentialsEncrypted) return reply.code(400).send({ error: "WHATSAPP_NOT_CONFIGURED", message: "Salve o bridge e os destinos do WhatsApp antes de validar." });
    ...
    await integrationRepository.recordValidation(integration.id, settings); return { data: result.data ?? [], allValid: destinations.every((destination) => validatedDestinations.includes(destination)) };
  });
};
