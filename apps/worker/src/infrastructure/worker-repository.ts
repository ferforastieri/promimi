...
export const workerRepository = {
  async automationPaused() {
    const control = await db.query.integrations.findFirst({
  ...
  delivered: (
    id: string,
    result: {
      status: "SENT" | "FAILED" | "PENDING" | "PAUSED";
      error?: string;
      externalId?: string;
    },
    attempts: number,
    ...
    db
      ...
      .where(eq(publications.id, id)),
  deliveryFailed: (id: string, attempts: number, error: string) =>
    db
      .update(publications)
      .set({ status: "FAILED", attempts, error, updatedAt: new Date() })
      .where(eq(publications.id, id)),
      ...
      .where(
        or(
          ...
          and(
            ...
            lt(publications.updatedAt, new Date(Date.now() - 5 * 60_000)),
          ),
        ),
      )
  ...
  expireOffers: () =>
    db
      ...
      .where(
        and(eq(offers.status, "PUBLISHED"), lt(offers.expiresAt, new Date())),
      ),
  scheduledRoutines: () =>
    db
      .select({
        ...
        scheduleCron: routines.scheduleCron,
      })
      .from(routines),
  queuedExecutions: () =>
    db
  ...
  executionStatus: (
    ...
    db
      ...
      .where(eq(routineExecutions.id, id)),
  enabledIntegrations: () =>
    db.select().from(integrations).where(eq(integrations.enabled, true)),
  offerBySource: (sourceUrl: string) =>
    db.query.offers.findFirst({ where: eq(offers.sourceUrl, sourceUrl) }),
  refreshOffer: (id: string, values: Partial<typeof offers.$inferInsert>) =>
    db
      .update(offers)
  ...
  createAutomatedOffer: (
    value: typeof offers.$inferInsert,
    destinations: string[],
  ) =>
    db.transaction(async (tx) => {
      const [offer] = await tx.insert(offers).values(value).returning();
